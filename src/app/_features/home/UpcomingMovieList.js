...
export const UpcomingMovieList = (F) => {
  const [upcomingMoviesData, setUpcomingMoviesData] = useState([]);

  const [loading, setLoading] = useState(false);

  const getUpcomingDataList = async () => {
    setLoading(true);
  ...
  function MovieCard(props) {
    ...
    return (
      <div className="w-[229.73px] h-[439px] bg-[#F4F4F5] rounded flex flex-col justify-end">
        <div style={{backgroundImage : `url('https://image.tmdb.org/t/p/original${imageURL}')`}} className={`w-[229.73px] h-[340px] bg-center bg-cover`}></div>
  ...
  return (
    <div className="  w-[1277px] h-[978px] flex flex-col justify-between">
      <div className="flex justify-between">
        <h3 className="font-semibold inter text-[24px]">Upcoming</h3>
